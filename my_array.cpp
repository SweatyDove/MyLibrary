...
my::Array<Type, size>& my::Array<Type, size>::operator=(const Array<Type, otherSize>& other)
{
    std::cout << "[my::Array]:[DEBUG]: copy assignment operator for the arrays of DIFFERENT sizes"
              << " has been called" << std::endl;    // ######## Just an info message (see the COMMENTS/BUGS section in the description above)
    if (size != otherSize) {
    ...
    // ######## right to left array
    unsigned int maxSize {(size < otherSize) ? size : otherSize};
